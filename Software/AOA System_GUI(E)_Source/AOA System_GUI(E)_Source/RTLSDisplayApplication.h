 ...
 * These pointers should not be used during initial initialization (in constructors),
 * as it is not guaranteed that they have been allocated and initialized yet.\n
 ...
 * The slot passed as the second argument will either run immediately if setup is already done,
 * or connected to a signal that will be emitted once initialization is complete.
 * @code
...
public:
    ...
    static serial_widget *serialSettings();
     ...
     * The function is either called right away if initialization is already done, or connected to the ready() signal.
